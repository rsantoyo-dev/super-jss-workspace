...
(function () {
  // Breakpoints aligned with SJSS defaults
  ...
  // Dev polling: in local dev, recompile periodically so saving public/hf.dsl.css updates live
  try {
    const host = location.hostname;
    if (host === 'localhost' || host === '127.0.0.1') {
      setInterval(() => { window.Headfire?.recompile?.() }, 1200);
    }
  } catch(_) {}
})();
